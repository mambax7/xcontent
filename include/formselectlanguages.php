...
class XoopsFormSelectLanguages extends XoopsFormElement
{
     ...
     */
    function XoopsFormSelectLanguages($caption, $name, $value = null, $size = 1, $multiple = false)
    {
		...
		foreach($this->GetLanguages() as $id => $lang) 
			$this->addOption(strtolower($lang), strtolower($lang));
    }
	...
	function GetLanguages(){
		...
		if ($GLOBALS['multilingual'] == true){
			$lang_handler =& xoops_getmodulehandler('xlanguage_ext', 'xcontent');
			$langs = $lang_handler->getObjects(NULL, true);
			foreach($langs as $id => $lang) {
				if (!in_array(strtolower($lang->getVar('lang_name')), $langs_array))
					$langs_array[] = strtolower($lang->getVar('lang_name'));
			}
		}
		return $langs_array;
	}
