...
use XoopsModules\Xcontent\Form\FormSelectCategories;
use XoopsModules\Xcontent\Form\FormSelectLanguages;
...
function xcontent_listblock()
{
    ...
    $limit = Request::getInt('limit', 30, 'GET');
    ...
    $ret    .= '</tr>';
    $blocks = $blockHandler->getObjects($criteria, true);
    if ($blocks) {
        $class = 'odd';
        /** @var \XoopsModules\Xcontent\Block $block */
        foreach ($blocks as $blockid => $block) {
            $class = ('odd' === $class) ? 'even' : 'odd';
            $ret   .= '<tr class="' . $class . '">';
            $ret   .= '<td>' . xcontent_getBlockTitle($blockid) . '</a></td>';
            $ret   .= '<td>' . date(_DATESTRING, $block->getVar('created')) . '</td>';
            $ret   .= '<td>' . $userHandler->getUnameFromId($block->getVar('uid')) . '</td>';
            $ret   .= '<td><a href="'
                      . XOOPS_URL
                      . '/modules/'
                      . _XCONTENT_DIRNAME
                      . xcontent_getpostinglocal()
                      . '?op='
                      . _XCONTENT_URL_OP_EDIT
                      . '&fct='
                      . _XCONTENT_URL_FCT_BLOCKS
                      . '&blockid='
                      . $blockid
                      . '"><img src="'
                      . XOOPS_URL
                      . '/modules/'
                      . _XCONTENT_DIRNAME
                      . '/assets/images/edit.png"></a>&nbsp;<a href="'
                      . XOOPS_URL
                      . '/modules/'
                      . _XCONTENT_DIRNAME
                      . xcontent_getpostinglocal()
                      . '?op='
                      . _XCONTENT_URL_OP_DELETE
                      . '&fct='
                      . _XCONTENT_URL_FCT_BLOCKS
                      . '&blockid='
                      . $blockid
                      ...
                      . '/modules/'
                      . _XCONTENT_DIRNAME
                      . '/assets/images/delete.png"></a>&nbsp;<a href="'
                      . XOOPS_URL
                      . '/modules/'
                      . _XCONTENT_DIRNAME
                      . xcontent_getpostinglocal()
                      . '?op='
                      . _XCONTENT_URL_OP_COPY
                      . '&fct='
                      . _XCONTENT_URL_FCT_BLOCKS
                      . '&blockid='
                      . $blockid
...
function xcontent_listcategory()
{
    ...
    if ($categories) {
        ...
        foreach ($categories as $catid => $category) {
            ...
            $formobj_catid      = new FormSelectCategories('', 'parent_id[' . $catid . ']', $category->getVar('parent_id'), 1, false, $catid);
...
function xcontent_addcategory($catid = 0, $language = '')
{
    ...
    if ($GLOBALS['xoopsModuleConfig']['multilingual'] && $GLOBALS['xoopsModuleConfig']['json']) {
        $sformobj['language']['sel'] = new FormSelectLanguages('', 'language', !empty($language) ? $language : $category['text']->getVar('language'));
        ...
        $sformobj['language']['sel'] = new FormSelectLanguages('', 'language', !empty($language) ? $language : $category['text']->getVar('language'));
        $sformobj['language']['sel']->setExtra('onChange="window.location=\'' . $_SERVER['SCRIPT_NAME'] . '?op=' . $_REQUEST['op'] . '&fct=' . $_REQUEST['fct'] . '&catid=' . $_REQUEST['catid'] . '&language=\'+document.category.language.options[document.category.language.selectedIndex].value"');
        $sformobj['language']['submit'] = new \XoopsFormButton('', 'submit_change', _SUBMIT);
        $sformobj['language']['submit']->setExtra('onClick="window.location=\'' . $_SERVER['SCRIPT_NAME'] . '?op=' . $_REQUEST['op'] . '&fct=' . $_REQUEST['fct'] . '&catid=' . $_REQUEST['catid'] . '&language=\'+document.category.language.options[document.category.language.selectedIndex].value"');
        $formobj['language'] = new \XoopsFormElementTray(_XCONTENT_AD_CAT_LANGUAGE, '&nbsp;');
        $formobj['language']->addElement($sformobj['language']['sel']);
        $formobj['language']->addElement($sformobj['language']['submit']);
    } else {
    ...
    $formobj['parent_id']        = new FormSelectCategories(_XCONTENT_AD_CAT_CATEGORYPARENT, 'parent_id', clear_unicodeslashes($category['cat']->getVar('parent_id')));
...
function xcontent_listxcontent()
{
    ...
    if ($xcontents) {
        ...
        foreach ($xcontents as $storyid => $xcontent) {
            ...
            $formobj_catid[$storyid]     = new FormSelectCategories('', 'catid[' . $storyid . ']', $xcontent->getVar('catid'));
            $ret                         .= '<td>' . $formobj_catid[$storyid]->render() . '</td>';
            $formobj_parent_id[$storyid] = new FormSelectPages('', 'parent_id[' . $storyid . ']', $xcontents[$storyid]->getVar('parent_id'), 1, false, $storyid);
...
function xcontent_addxcontent($storyid = 0, $language = '')
{
    ...
    if ($GLOBALS['xoopsModuleConfig']['multilingual'] && $GLOBALS['xoopsModuleConfig']['json']) {
        $sformobj['language']['sel'] = new FormSelectLanguages('', 'language', !empty($language) ? $language : $xcontent['text']->getVar('language'));
        $sformobj['language']['sel']->setExtra('onChange="javascript:doJSON_LoadPageForm();"');
        $sformobj['language']['submit'] = new \XoopsFormButton('', 'submit_change', _SUBMIT);
        $sformobj['language']['submit']->setExtra('onClick="javascript:doJSON_LoadPageForm();"');
        ...
        $sformobj['language']['sel'] = new FormSelectLanguages('', 'language', !empty($language) ? $language : $xcontent['text']->getVar('language'));
        $sformobj['language']['sel']->setExtra('onChange="window.location=\'' . $_SERVER['SCRIPT_NAME'] . '?op=' . $_REQUEST['op'] . '&fct=' . $_REQUEST['fct'] . '&storyid=' . $_REQUEST['storyid'] . '&language=\'+document.xcontent.language.options[document.xcontent.language.selectedIndex].value"');
        $sformobj['language']['submit'] = new \XoopsFormButton('', 'submit_change', _SUBMIT);
        $sformobj['language']['submit']->setExtra(
            'onClick="window.location=\'' . $_SERVER['SCRIPT_NAME'] . '?op=' . $_REQUEST['op'] . '&fct=' . $_REQUEST['fct'] . '&storyid=' . $_REQUEST['storyid'] . '&language=\'+document.xcontent.language.options[document.xcontent.language.selectedIndex].value"'
        );
        $formobj['language'] = new \XoopsFormElementTray(_XCONTENT_AD_LANGUAGE, '&nbsp;');
        $formobj['language']->addElement($sformobj['language']['sel']);
        $formobj['language']->addElement($sformobj['language']['submit']);
    ...
    $formobj['weight'] = new \XoopsFormText(_XCONTENT_AD_WEIGHT, 'weight', 10, 4, $_GET['weight'] ?? clear_unicodeslashes($xcontent['xcontent']->getVar('weight')));

    $formobj['title'] = new \XoopsFormText(_XCONTENT_AD_MENUTITLE, 'title', 45, 128, $_GET['title'] ?? clear_unicodeslashes($xcontent['text']->getVar('title')));

    $formobj['ptitle'] = new \XoopsFormText(_XCONTENT_AD_PAGETITLE, 'ptitle', 45, 128, $_GET['ptitle'] ?? clear_unicodeslashes($xcontent['text']->getVar('ptitle')));

    $formobj['parent_id']        = new FormSelectPages(_XCONTENT_AD_PARENT, 'parent_id', $_GET['parent_id'] ?? $xcontent['xcontent']->getVar('parent_id'), 1, false, $xcontent['xcontent']->getVar('story_id'));
    $formobj['catid']            = new FormSelectCategories(_XCONTENT_AD_CATEGORY, 'catid', $_GET['catid'] ?? $xcontent['xcontent']->getVar('catid'));
    $formobj['blockid']          = new FormSelectBlocks(_XCONTENT_AD_INHERITBLOCK, 'blockid', $_GET['blockid'] ?? $xcontent['xcontent']->getVar('blockid'));
    $formobj['keywords']         = new \XoopsFormTextArea(_XCONTENT_AD_KEYWORDS, 'keywords', $_GET['keywords'] ?? clear_unicodeslashes($xcontent['text']->getVar('keywords')), 5, 45);
    $formobj['page_description'] = new \XoopsFormTextArea(_XCONTENT_AD_PAGEDESCRIPTION, 'page_description', $_GET['page_description'] ?? clear_unicodeslashes($xcontent['text']->getVar('page_description')), 5, 45);
    ...
    //          $formobj['template'] = new \XoopsModules\Xcontent\Form\FormSelectTemplates(_XCONTENT_AD_TEMPLATES, 'template', $_GET['template']);
    ...
    //          $formobj['template'] = new \XoopsModules\Xcontent\Form\FormSelectTemplates(_XCONTENT_AD_TEMPLATES, 'template', $_GET['template']);
    //          $formobj['template']->setExtra('onChange="window.location=\''.$_SERVER['SCRIPT_NAME'].'?op='.$_REQUEST['op'].'&fct='.$_REQUEST['fct'].'&storyid='.$_REQUEST['storyid'].'&template=\'+document.xcontent.template.options[document.xcontent.template.selectedIndex].value+\'&language=\'+document.xcontent.language.options[document.xcontent.language.selectedIndex].value"');
    //      } elseif ($xcontent['text']->isNew()&&!$GLOBALS['xoopsModuleConfig']['json']&&!$GLOBALS['xoopsModuleConfig']['multilingual']) {
    //          $formobj['template'] = new \XoopsModules\Xcontent\Form\FormSelectTemplates(_XCONTENT_AD_TEMPLATES, 'template', $_GET['template']);
    //          $formobj['template']->setExtra('onChange="window.location=\''.$_SERVER['SCRIPT_NAME'].'?op='.$_REQUEST['op'].'&fct='.$_REQUEST['fct'].'&storyid='.$_REQUEST['storyid'].'&template=\'+document.xcontent.template.options[document.xcontent.template.selectedIndex].value"');
    //
    //      }
    ...
    $sformobj['url']['txt']   = new \XoopsFormText(_XCONTENT_AD_URLADDRESS, 'address', 45, 198, $_GET['address'] ?? $xcontent['xcontent']->getVar('address'));
    $sformobj['url']['radio'] = new \XoopsFormRadioYN(_XCONTENT_AD_REDIRECTLINK, 'link', $_GET['link'] ?? $xcontent['xcontent']->getVar('link'));
    ...
    $sformobj['publish']['date'] = new \XoopsFormDateTime(_XCONTENT_AD_PUBlISHDATETIME, 'publish', 15, $_GET['publish'] ?? $xcontent['xcontent']->getVar('publish'));
    $sformobj['publish']['page'] = new FormSelectPages(_XCONTENT_AD_REDIRECTPAGE, 'publish_storyid', $_GET['publish_storyid'] ?? $xcontent['xcontent']->getVar('publish_storyid'), 1, false, $xcontent['xcontent']->getVar('story_id'));
    ...
    $sformobj['expire']['date'] = new \XoopsFormDateTime(_XCONTENT_AD_EXPIREDATETIME, 'expire', 15, $_GET['expire'] ?? $xcontent['xcontent']->getVar('expire'));
    $sformobj['expire']['page'] = new FormSelectPages(_XCONTENT_AD_REDIRECTPAGE, 'expire_storyid', $_GET['expire_storyid'] ?? $xcontent['xcontent']->getVar('expire_storyid'), 1, false, $xcontent['xcontent']->getVar('story_id'));
    ...
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_VISIBLE, 'visible', $_GET['visible'] ?? $xcontent['xcontent']->getVar('visible')));
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_HOMEPAGE, 'homepage', $_GET['homepage'] ?? $xcontent['xcontent']->getVar('homepage')));
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_NOHTML, 'nohtml', $_GET['nohtml'] ?? $xcontent['xcontent']->getVar('nohtml')));
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_NOSMILEY, 'nosmiley', $_GET['nosmiley'] ?? $xcontent['xcontent']->getVar('nosmiley')));
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_NOBREAKS, 'nobreaks', $_GET['nobreaks'] ?? $xcontent['xcontent']->getVar('nobreaks')));
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_NOCOMMENTS, 'nocomments', $_GET['nocomments'] ?? $xcontent['xcontent']->getVar('nocomments')));
    $eletray['options']->addElement(new \XoopsFormRadioYN(_XCONTENT_AD_SUBMENU, 'submenu', $_GET['submenu'] ?? $xcontent['xcontent']->getVar('submenu')));
    $formobj['options'] = $eletray['options'];
    ...
    $sform->addElement(new \XoopsFormHidden('xcontentid', $_GET['xcontentid'] ?? $xcontent['text']->getVar('xcontentid')));
    $sform->addElement(new \XoopsFormHidden('storyid', $_GET['storyid'] ?? $xcontent['xcontent']->getVar('storyid')));
    $sform->addElement(new \XoopsFormHidden('op', _XCONTENT_URL_OP_SAVE));
...
function xcontent_passwordform($storyid = 0)
{
    ...
    $sform->addElement(new \XoopsFormHidden('storyid', $_GET['storyid'] ?? $storyid));
...
function xcontent_addblock($blockid = 0, $language = '')
{
    ...
    if ($GLOBALS['xoopsModuleConfig']['multilingual'] && $GLOBALS['xoopsModuleConfig']['json']) {
        ...
        $sformobj['language']['sel'] = new FormSelectLanguages('', 'language', !empty($language) ? $language : $category['text']->getVar('language'));
        $sformobj['language']['sel']->setExtra('onChange="window.location=\'' . $_SERVER['SCRIPT_NAME'] . '?op=' . $_REQUEST['op'] . '&fct=' . $_REQUEST['fct'] . '&blockid=' . $_REQUEST['blockid'] . '&language=\'+document.block.language.options[document.block.language.selectedIndex].value"');
        $sformobj['language']['submit'] = new \XoopsFormButton('', 'submit_change', _SUBMIT);
        $sformobj['language']['submit']->setExtra('onClick="window.location=\'' . $_SERVER['SCRIPT_NAME'] . '?op=' . $_REQUEST['op'] . '&fct=' . $_REQUEST['fct'] . '&blockid=' . $_REQUEST['blockid'] . '&language=\'+document.block.language.options[document.block.language.selectedIndex].value"');
        $formobj['language'] = new \XoopsFormElementTray(_XCONTENT_AD_CAT_LANGUAGE, '&nbsp;');
        $formobj['language']->addElement($sformobj['language']['sel']);
        $formobj['language']->addElement($sformobj['language']['submit']);
    } else {
    ...
    $formobj['title'] = new \XoopsFormText(_XCONTENT_AD_OPENDESCRIPTION, 'title', 45, 128, $_GET['title'] ?? clear_unicodeslashes($block['text']->getVar('title')));
    //$formobj['title']->setExtra('onChange="javascript:doJSON_CheckForm();"');

    $page_desc_configs           = [];
    $page_desc_configs['name']   = 'text';
    $page_desc_configs['value']  = $_GET['text'] ?? clear_unicodeslashes($block['text']->getVar('text'));
    $page_desc_configs['rows']   = 35;
    $page_desc_configs['cols']   = 60;
    $page_desc_configs['width']  = '100%';
    $page_desc_configs['height'] = '400px';
    ...
    $sform->addElement(new \XoopsFormHidden('xcontentid', $_GET['xcontentid'] ?? $block['text']->getVar('xcontentid')));
    $sform->addElement(new \XoopsFormHidden('blockid', $_GET['blockid'] ?? $block['block']->getVar('blockid')));
    $sform->addElement(new \XoopsFormHidden('op', _XCONTENT_URL_OP_SAVE));
...
function xcontent_listuserxcontent()
{
    ...
    if ($xcontents) {
        ...
        foreach ($xcontents as $storyid => $xcontent) {
            ...
            $ret                         .= '<tr class="' . $class . '">';
            $ret                         .= '<td><a href="' . XOOPS_URL . '/modules/' . _XCONTENT_DIRNAME . '/?storyid=' . $storyid . '">' . xcontent_getTitle($storyid) . '</a></td>';
            $formobj_catid[$storyid]     = new FormSelectCategories('', 'catid[' . $storyid . ']', $xcontent->getVar('catid'), 1, false, $story_id);
            $ret                         .= '<td>' . $formobj_catid[$storyid]->render() . '</td>';
            $formobj_parent_id[$storyid] = new FormSelectPages('', 'parent_id[' . $storyid . ']', $xcontents[$storyid]->getVar('parent_id'));
            $ret                         .= '<td>' . $formobj_parent_id[$storyid]->render() . '</td>';
            $formobj_submenu[$storyid]   = new \XoopsFormRadioYN('', 'submenu[' . $storyid . ']', $xcontents[$storyid]->getVar('submenu'));
            $ret                         .= '<td>' . $formobj_submenu[$storyid]->render() . '</td>';
            $formobj_homepage[$storyid]  = new \XoopsFormRadioYN('', 'homepage[' . $storyid . ']', $xcontents[$storyid]->getVar('homepage'));
