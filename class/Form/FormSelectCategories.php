 ...
 * @access      public
 */
class FormSelectCategories extends \XoopsFormElement
{
    /**
     * Options
     *
     ...
     * @return mixed
     */
    public function TreeMenu($langs_array, $categories, $level = 0, $ownid = 0)
    {
        ++$level;
        $helper          = Helper::getInstance();
        $categoryHandler = $helper->getHandler(\_XCONTENT_CLASS_CATEGORY);
        foreach ($categories as $catid => $category) {
            if ($catid != $ownid) {
                ...
                $categoriesb         = $categoryHandler->getObjects(new \Criteria('parent_id', $catid), true);
                if ($categoriesb) {
