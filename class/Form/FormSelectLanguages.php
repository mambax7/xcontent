...
namespace XoopsModules\Xcontent\Form;
 ...
 * @access      public
 */
class FormSelectLanguages extends \XoopsFormElement
{
    /**
     * Options
     *
    ...
    public function __construct($caption, $name, $value = null, $size = 1, $multiple = false)
    {
        ...
        foreach ($this->getLanguages() as $id => $lang) {
            $this->addOption(mb_strtolower($lang), mb_strtolower($lang));
        }
    }
    ...
    public function getLanguages()
    {
        ...
        if (true === $GLOBALS['multilingual']) {
            ...
            /** @var \XoopsModules\Xcontent\XlanguageExtHandler $langHandler */
            $langHandler = $helper->getHandler('XlanguageExt');
            $langs       = $langHandler->getObjects(null, true);
            foreach ($langs as $id => $lang) {
                if (!\in_array(mb_strtolower($lang->getVar('lang_name')), $langs_array)) {
                    $langs_array[] = mb_strtolower($lang->getVar('lang_name'));
                }
            }
        }
