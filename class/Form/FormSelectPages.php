...
/**
 * Class XoopsModules\Xcontent\FormSelectPages
 */
class FormSelectPages extends \XoopsFormElement
{
    /**
     * Options
     *
    ...
    /**
     * XoopsModules\Xcontent\FormSelectPages constructor.
     * @param      $caption
     * @param      $name
     * @param null $value
     * @param int  $size
    ...
    public function TreeMenu($langs_array, $xcontents, $level, $ownid)
    {
        ...
        foreach ($xcontents as $storyid => $xcontent) {
            if ($storyid != $ownid) {
                ...
                $xcontentsb            = $xcontentHandler->getObjects(new \Criteria('parent_id', $storyid), true);
                if ($xcontentsb) {
