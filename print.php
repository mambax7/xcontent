...
if ($xcontent = $xcontentHandler->getContent((int)$storyid)) {
    if (!$gpermHandler->checkRight(_XCONTENT_PERM_MODE_VIEW . _XCONTENT_PERM_TYPE_XCONTENT, $xcontent['xcontent']->getVar('storyid'), $groups, $modid)) {
        ...
        if ($GLOBALS['xoopsModuleConfig']['htaccess']) {
            if (strpos($_SERVER['REQUEST_URI'], 'odules/') > 0) {
                ...
                if ($category['text']->getVar('title') != '') {
                    ...
                    header('HTTP/1.1 301 Moved Permanently');
                    header('Location: ' . XOOPS_URL . '/' . $GLOBALS['xoopsModuleConfig']['baseurl'] . '/print,' . $storyid . $GLOBALS['xoopsModuleConfig']['endofurl']);
                }
                exit(0);
            }
        }
        ...
        echo '  <link rel="stylesheet" type="text/css" media="screen" href="' . XOOPS_URL . '/modules/xcontent/assets/css/print.css">';
        echo '</head>';
